...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: previewRows }, { data: revisions }, { data: events }] = await Promise.all([
    supabaseAdmin.from("previews").select("id,version_number,storage_path,label,created_at").eq("order_id", id).order("version_number"),
    supabaseAdmin.from("revision_requests").select("id,preview_id,message,status,created_at,completed_at").eq("order_id", id).order("created_at", { ascending: false }),
    supabaseAdmin.from("audit_events").select("id,event_type,event_data,created_at").eq("order_id", id).order("created_at", { ascending: false }).limit(30),
  ]);
