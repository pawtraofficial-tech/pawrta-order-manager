...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { data: existing } = await supabaseAdmin.from("previews").select("version_number").eq("order_id", orderId).order("version_number", { ascending: false }).limit(1);
    ...
    const upload = await supabaseAdmin.storage.from("previews").upload(path, Buffer.from(await file.arrayBuffer()), { contentType: file.type, upsert: false });
    if (upload.error) throw upload.error;
    const insert = await supabaseAdmin.from("previews").insert({ order_id: orderId, version_number: version, storage_path: path, label: version === 1 ? "Initial Design" : `Revision ${version - 1}` }).select("id").single();
    if (insert.error) { await supabaseAdmin.storage.from("previews").remove([path]); throw insert.error; }

    const now = new Date().toISOString();
    await supabaseAdmin.from("orders").update({ status: "preview_ready", updated_at: now }).eq("id", orderId);
    await audit(orderId, "preview_uploaded", { previewId: insert.data.id, version });
    const signed = await supabaseAdmin.storage.from("previews").createSignedUrl(path, 60 * 60 * 24 * 7);
