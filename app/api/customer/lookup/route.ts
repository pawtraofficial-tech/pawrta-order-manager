...
export async function POST(req: NextRequest) {
  const limited = rateLimit(req, "customer-lookup", 20, 10 * 60 * 1000);
  ...
  const { data: order, error: orderError } = await db.from("orders").select("id,order_number,customer_name,status,revision_count,approved_preview_id,approved_at,production_ready,updated_at").eq("order_number", orderNumber).eq("customer_email", email).maybeSingle();
  if (orderError) {
  ...
  const [previewResult, revisionResult] = await Promise.all([
    db.from("previews").select("id,version_number,storage_path,label,created_at").eq("order_id", order.id).order("version_number"),
