...
export async function POST(req: NextRequest) {
  ...
  const { data: preview } = await getSupabaseAdmin().from("previews").select("id,storage_path").eq("id", parsed.data.previewId).eq("order_id", order.id).maybeSingle();
  if (!preview) return NextResponse.json({ error: "Selected preview does not belong to this order." }, { status: 400 });
  const { data: openRevision } = await getSupabaseAdmin().from("revision_requests").select("id").eq("order_id", order.id).eq("status", "open").limit(1);
  if (openRevision?.length) return NextResponse.json({ error: "A revision request is still open." }, { status: 409 });

  const now = new Date().toISOString();
  const update = await getSupabaseAdmin().from("orders").update({ status: "approved", approved_preview_id: preview.id, approved_at: now, production_ready: true, updated_at: now }).eq("id", order.id).is("approved_at", null);
  ...
  const signed = await getSupabaseAdmin().storage.from("previews").createSignedUrl(preview.storage_path, 60 * 60 * 24 * 7);
