...
export async function POST(req: NextRequest) {
  ...
  const { data: preview } = await supabaseAdmin.from("previews").select("id").eq("id", parsed.data.previewId).eq("order_id", order.id).maybeSingle();
  if (!preview) return NextResponse.json({ error: "Selected preview does not belong to this order." }, { status: 400 });
  const { data: open } = await supabaseAdmin.from("revision_requests").select("id").eq("order_id", order.id).eq("status", "open").limit(1);
  ...
  const request = await supabaseAdmin.from("revision_requests").insert({ order_id: order.id, preview_id: preview.id, message: parsed.data.message }).select("id").single();
  if (request.error) return NextResponse.json({ error: "Revision request failed." }, { status: 500 });
  const now = new Date().toISOString();
  await supabaseAdmin.from("orders").update({ status: "revision_requested", revision_count: next, updated_at: now }).eq("id", order.id);
